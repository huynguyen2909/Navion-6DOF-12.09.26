// main.cpp
//
// Everything t6c_fuselage.hpp/.cpp deliberately leaves out: real numbers
// (hand-transcribed from t6c_fuselage_params.yaml, and from Nicolosi's own
// paper for the regression test), and two things to run:
//
...
//                 docs/model_formulas.pdf Sec.3.
//   (default)     Sweeps V for the T-6C's own (partly ESTIMATED) geometry
//                 and prints CD/CM/CN and the resulting forces/moments.
//
// No visualization, no real-time loop -- this model has no per-step state
// (see t6c_fuselage.hpp), so there is nothing to animate.

#include <cmath>
#include <cstdio>
#include <cstring>

#include "t6c_fuselage.hpp"
...
t6c_fuselage::Geometry MakeNicolosiReferenceGeometry() {
  t6c_fuselage::Geometry g;
  g.Lf_m = 30.0;
  g.df_m = 3.45;
  g.Ln_m = 5.7;
  g.Lc_m = 13.0;
...
// T-6C geometry, hand-transcribed from t6c_fuselage_params.yaml -- see that
// file for which fields are VERIFIED vs ESTIMATED and why.
t6c_fuselage::Geometry MakeT6CGeometry() {
  t6c_fuselage::Geometry g;
  g.Lf_m = 10.18;
  g.df_m = 1.05;
  g.Ln_m = 1.93;
...
// Tuning, hand-transcribed from t6c_fuselage_params.yaml's `tuning:` block.
// Every pitch/yaw field here is UNCALIBRATED -- see that file and
// docs/model_formulas.pdf Sec.11 ("Calibration workflow").
t6c_fuselage::TuningParams MakeT6CTuning() {
  t6c_fuselage::TuningParams t;
  t.windshield_drag_increment = 0.0;

  t6c_fuselage::PitchTuning& p = t.pitch;
  ...
  t6c_fuselage::YawTuning& y = t.yaw;
...
void RunRegressionTest() {
  ...
  t6c_fuselage::FuselageModel model(MakeNicolosiReferenceGeometry(),
                                     t6c_fuselage::TuningParams{});  // drag needs no tuning

  t6c_fuselage::AeroState state;
  ...
  const t6c_fuselage::Output out = model.Update(state);
...
// ---------------------------------------------------------------------
void RunT6CSweep() {
  std::printf("=== T-6C fuselage sweep (geometry partly ESTIMATED -- see\n");
  std::printf("    t6c_fuselage_params.yaml) ===\n");

  t6c_fuselage::FuselageModel model(MakeT6CGeometry(), MakeT6CTuning());
  ...
  std::printf("\n(CM/CN use UNCALIBRATED trend-guessed slopes -- do not treat as\n"
              " validated; see docs/model_formulas.pdf Sec.11.)\n");
}
