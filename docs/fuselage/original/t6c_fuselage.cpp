...
double FrontalArea(const Geometry& g) { return (M_PI / 4.0) * g.df_m * g.df_m; }

double NoseWettedArea(const Geometry& g) {
  const double slant = std::sqrt(g.Ln_m * g.Ln_m + (g.df_m / 2.0) * (g.df_m / 2.0));
  return M_PI * (g.df_m / 2.0) * slant;
}

double CabinWettedArea(const Geometry& g) { return M_PI * g.df_m * g.Lc_m; }

double TailWettedArea(const Geometry& g) {
  const double half_taper = (g.df_m - g.db_m) / 2.0;
  const double slant = std::sqrt(g.Lt_m * g.Lt_m + half_taper * half_taper);
  return M_PI * (g.df_m / 2.0 + g.db_m / 2.0) * slant;
}

}  // namespace

FuselageModel::FuselageModel(Geometry geometry, TuningParams tuning)
...
Output FuselageModel::Update(const AeroState& state) const {
  Output out;
  ...
  const PitchTuning& pt = tuning_.pitch;
  ...
  const YawTuning& yt = tuning_.yaw;
